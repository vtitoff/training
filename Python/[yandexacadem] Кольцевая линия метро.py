...
def metro(N, i, j):
    ...
    straight_stations=list(stations[i:])+stations[:i-1]
    back_stations=list(reversed(stations[:i-1]))+list(reversed(stations[i:]))
    s=0
    b=0
    for i in straight_stations:
        if i==j:
            break
        else:
            s+=1
    for i in back_stations:
        if i==j:
            break
        else:
            b+=1
